<div class="_status" >
    ...
    <div class="_status__header">
        ...
        @if (!empty($_booking->BookingDetails->BookingId) && !empty($cancelllations[$_booking->BookingDetails->BookingId]))
            @php
            ...
            @if (!empty($_booking->BookingDetails->BookingCurrency) && !empty($_booking->BookingDetails->BookingPrice))
                <div class="col-sm-4">
                    <div class="_status__info__title">Price
                        <span class="_status__info__headline">
                            {{ $_booking->BookingDetails->BookingCurrency }}
                            {{-- {{ ceil($_booking->BookingDetails->BookingPrice) }} --}}

                            @if (!empty($_booking->hotel_hike))
                                @php
                                    $_total = 0;
                                    $_prices = explode('|', $_booking->BookingDetails->BookingPrice);
                                    foreach ($_prices as $_price) {
                                        // $_price = ceil($_price);
                                        $_price = ceil(\App\Helpers\PriceHelper::getHikedHotelPrice($_price, Auth::user(), $_booking->hotel_hike));
                                        $_total += $_price;
                                    }
                                @endphp
                                {{-- {{ ceil(\App\Helpers\PriceHelper::getHikedHotelPrice($_booking->BookingDetails->BookingPrice, Auth::user(), $_booking->hotel_hike)) }} --}}
                                {{ $_total }}
                            @else
                                {{ ceil($_booking->BookingDetails->BookingPrice) }}
                            @endif
    ...
    @foreach ($_booking->BookingRequest->Booking->RoomDetails as $_RoomDetail)
                    ...
                    <div class="row">
                        <div class="col-xs-5">
                            <div class="_status__table__td">
                                Room #{{ $i + 1 }}
                                <span class="_status__table__td ">{{ $types[$i] }}</span>
                            </div>
                        </div>
                        <div class="col-xs-2">
                            <div class="_status__table__td text-center">
                                @if (!empty($_booking->hotel_hike))
                                    {{ ceil(\App\Helpers\PriceHelper::getHikedHotelPrice($rates[$i], Auth::user(), $_booking->hotel_hike)) }}
                                @else
                                    {{ ceil($rates[$i]) }}
                                @endif
                            </div>
        ...
        @foreach ($_RoomDetail->Guests as $_guest)
		  ...
		  @if(!empty($_booking->TermsAndConditions))
		<div class="_status__table__head secondary">
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="_status__table__th">Terms And Conditions</div>
                        </div>
                    </div>
                </div>
                <div class="_status__table__body">
                    <div class="row">
                        <div class="col-xs-12">
                            <div class="_status__table__td">{{$_booking->TermsAndConditions}}</div>
                        </div>
                    </div>
                </div>
		@endif
            ...
            @if( $booking->is_paid)
                @if (!empty($_booking->BookingDetails->BookingId))
                    <div class="col-xs-12 text-center">
                        <a target="_blank" href="{{ route('booking.invoice', [$booking->id, $_booking->BookingDetails->BookingId]) }}" class="btn ticker-btn ">Invoice</a>&nbsp;&nbsp;
                        @php
                            // dd($_booking)
                        @endphp
                        <a target="_blank" href="{{ route('booking.voucher', [$booking->id, $_booking->BookingDetails->BookingId]) }}" class="btn ticker-btn ">Voucher</a>
                    </div>
                @endif
            @endif
        </div>
